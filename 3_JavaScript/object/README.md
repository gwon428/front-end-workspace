...
```js
// RegExp 생성자 함수
const regExp1 = new RegExp("패턴"); // 패턴이 있는지 없는지를 판단할 때 사용하는 게 정규표현식
// 정규표현식 리터럴
const regExp2 = /패턴/;
```
...
- 정규식.exec(문자열) : 문자열에서 정규식 값과 일치하는 값이 있으면 처음 매칭된 문자열, 없으면 null을 리턴

### 문자열(new String을 통해 생성) 객체에서 제공하는 함수

(문자열도 객체임 (String 객체)!!!!!!!!!!!!!)

- 문자열.match(정규표현식) : 문자열에서 정규식 값과 일치하는 값을 찾아서 리턴
- 문자열.search(정규표현식) : 문자열에서 정규식 값과 일치하는 값의 시작 인덱스 값을 리턴, 없으면 -1을 리턴
- 문자열.replace(정규표현식, 변경할 값) : 문자열에서 정규식 값과 일치하는 첫번째 부분을 바꿀 값으로 변경해서 리턴
- 문자열.split(정규표현식) : 정규식에 지정된 값을 구분자로 해서 쪼개진 값들이 담겨있는 배열 리턴
...
- 메타 문자를 이용해서 정규표현식으로 검색조건을 삼을 "특정 패턴"을 제시 가능
