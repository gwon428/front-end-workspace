// 1. Filtering 메서드
// 선택된 요소들 중 첫번째 요소를 가져와서 스타일 변경 (first)
...
$("h4").filter(":last").css("background-color", "beige");

// 선택된 요소들 중 세번째 요소를 가져와서 스타일 변경 : eq (index 0부터 시작)
$("h4").eq(2).css("background-color", "skyblue");
$("h4").filter(":eq(2)").text("eq에 의해 선택되었습니다.");

// 선택된 요소들 중 홀수 인덱스에 요소를 가져와서 스타일 변경
// 홀수 (odd) / 짝수 (even)
$("h4").filter(":odd").css({"background-color":"hotpink", "color":"white"});

// 선택된 요소들 중 특정 요소가 아닌 요소들을 선택해서 스타일 변경
// $("h4").not(".test")
// $("h4:not(.test)")
$("h4").filter(":not(.test)").css({"text-decoration": "underline", "background-color":"orange"});
...
// span 요소의 부모 요소에 스타일 적용 : 글자색 red, 테드리 : 2px solid
$("span").parent().css({color:"red", border:"2px solid"});

// li 요소의 모든 상위 요소에 스타일 적용 : 글자색 - blue
$("li").parents().css("color", "blue");

// li 요소의 상위 요소 중 div만 스타일 적용 : 테두리 : 2px dashed tomato
$("li").parents("div").css("border", "2px dashed tomato");

// span 요소부터 div 요소 이전까지 상위 요소 스타일 설정 : 배경색 lavenderblush
...
// 자식의 자식 요소 중 ul 태그에 style3 적용 (자식 요소 중?..)
$(".wrap").children().children("ul").css(style3);

// 자식의 자식의 자식 요소 중 li 태그의 style4 적용 (자식의 자식 요소 중?..)
...
// h2 요소의 이전에 오는 모든 (prevAll) 형제 요소에 배경색 : moccasin
